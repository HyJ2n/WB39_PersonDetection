# 특정 얼굴 트래킹 성공 가장 안뺏기는 버전이긴함 (준호는 스틸해감)
...
class FaceRecognizer:
    ...
    def calculate_iou(self, bbox1, bbox2):
        ...
        yi2 = min(y2, y4)  # 교차하는 부분의 우측 하단 y 좌표
        ...
        print(f"박스1 좌표 : {bbox1_area}")
        print(f"박스2 좌표 : {bbox2_area}")
    ...
    def assign_face_id(self, face_encoding, known_faces, threshold=0.65):
        if known_faces:
    ...
    def recognize_faces(self, frame, frame_number, output_dir, known_faces, tracker, video_name, yolo_model, gender_model, age_model):
        ...
        for track in tracks:
            ...
            if bbox_area / frame_area <= 0.021:
                # Bounding box 크기가 전체 프레임의 10% 이하인 경우 트래킹 중지
        ...
        for track in tracks:
            ...
            for other_track in tracks:
                if other_track.track_id != track_id:
                    ...
                    if overlap_area / track_area >= 0.8:  # 겹치는 영역이 80% 이상인 경우
                        overlapping_track_ids.add(track_id)  # 겹치는 트랙 ID 추가
                        overlapping_track_ids.add(other_track.track_id)  # 다른 트랙 ID도 추가
                        break

            if track_id in overlapping_track_ids:
                continue  # 겹치는 경우 트랙킹 업데이트를 건너뜀
