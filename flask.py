...
def tracking_video():
    try:
        process = subprocess.Popen(["python", "tracking_final6.py"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
#트래킹 영상 DB에 저장 함수
def save_processed_video_info(video_name, video_folder, person_id):
    connection = get_db_connection()
    try:
        with connection.cursor() as cursor:
            pro_video_name = f"{person_id}_output.mp4"
            pro_video_path = os.path.join(video_folder, pro_video_name)
...
#얼굴 처리 함수
def process_save_face_info():
    try:
        # save_face_info6.py 스크립트 호출 (백그라운드 실행)
        process = subprocess.Popen(["python", "save_face_info6.py"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        stdout, stderr = process.communicate()
...
def process_video():
    try:
        ...
        process = subprocess.Popen(["python", "Main_image.py"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        stdout, stderr = process.communicate()
