# 트래킹 객체 붙이기 까지 성공 ( Person_id에 따른 고정은 아직X )
...
class FaceRecognizer:
    ...
    def recognize_faces(self, frame, frame_number, output_dir,known_face,tracker, video_name, yolo_model, gender_model, age_model):
        ...
        if detect_faces:
            for embedding, box, face_image, prob in detect_faces:  # 영상에서 검출된 얼굴 위치, 임베딩값 전부 가져옴
                ...
                for track in tracks:
                    ...
                    cv2.rectangle(frame, (track_bbox[0], track_bbox[1]), (track_bbox[2], track_bbox[3]), (0, 255, 0), 2)  # Green box for tracking
...
if __name__ == "__main__":
    video_paths = ["./test/testVideo2.mp4"] #, "./test/testVideo2.mp4"]
    known_face_paths = [
        "./test/horyun.png"
    ]
